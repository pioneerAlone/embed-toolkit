...
embed-toolkit is a Claude Code skill pack for embedded firmware development. It provides 11 skills (`embed-build`, `embed-flash`, `embed-serial`, `embed-debug`, `embed-diag`, `embed-workflow`, `embed-setup`, `embed-test`, `embed-memory`, `embed-crash`, `embed-static`) that auto-detect the host toolchain and MCU platform, then execute the right commands.
